...
namespace aedlf {
    ...
    class Matrix{
        public:
            ...
            using slice_parma = std::vector<int>;
            using ul_pos = const std::pair<int, int>;
            Matrix();
            ~Matrix();
            Matrix(matrix_dim shape, MType fill_with);
            Matrix(matrix_data_p data, matrix_dim shape); // 这里仿照caffe使用1d-array
            Matrix(matrix_dim shape, std::initializer_list<MType> init_data);
            Matrix(std::initializer_list<int> shape, std::initializer_list<MType> init_data);
            Matrix(std::initializer_list<int> shape, MType fill_with);
            Matrix(const Matrix<MType>& m);
            Matrix<MType>& operator=(const Matrix<MType>& m);
            Matrix<MType>& operator+(const Matrix<MType>& m);
            Matrix<MType>& operator+(MType number);
            Matrix<MType>& operator*(int scale_number);
            Matrix<MType>& operator*(const Matrix<MType>& m);
            Matrix<MType>& operator+=(const Matrix<MType>& m);
            Matrix<MType>& operator+=(MType number);
            Matrix<MType>& operator*=(const Matrix<MType>& m);
            ...
            Matrix<MType>& scale(int scale_number);
            void clear_data();
            void set_data(matrix_data_p data);
            void resize(matrix_dim shape, MType fill_with);
            void resize(int n, int c, int h, int w, MType fill_with);
            void concat(Matrix<MType>& m, unsigned dim);
            Matrix<MType> sum_by_dim(int sum_dim);
            Matrix<MType> slice(std::initializer_list<int> slice_index);
            Matrix<MType> slice(slice_parma slice_index);
            Matrix<MType> slice(int slice_dim, int start, int end);
            Matrix<MType> slice(int slice_dim, std::initializer_list<int> range);
            void regard_as(matrix_dim shape);
            void regard_as(std::initializer_list<int> shape);
            void restore_shape();
            bool is_regarded_as();
            void T();
            bool is_uninitialized();
            MType get(int n, int c, int h, int w);
            MType get(int index);
            void set(int n, int c, int h, int w, MType value);
            void set(int index, MType value);
            ul_pos get_batch(matrix_dim dim, int batch_id) const;
            ul_pos get_batch(int batch_id) const;
            ul_pos get_channel(matrix_dim dim, int channel_id, ul_pos batch_pos) const;
            ul_pos get_channel(int channel_id, ul_pos batch_pos) const;
        ...
        private:
            ...
            int T_core_get_next(int now_index, int m_h, int m_len);
            void sum_by_dim_core(Matrix<MType>& m, Matrix<MType>& result, int sum_dim, int batch_id);
            void concat_core(Matrix<MType>& m, Matrix<MType>& result, unsigned dim, int batch_id);
            matrix_data_p data;
            matrix_dim shape; // (n,c,h,w)
            matrix_dim shape_backup {0,0,0,0};
    ...
    template <typename MType>
    Matrix<MType>::Matrix(std::initializer_list<int> shape, std::initializer_list<MType> init_data) {
        assert(shape.size() == 4);
        new (this)Matrix(matrix_dim {shape}, init_data);
    }

    template <typename MType>
    Matrix<MType>::Matrix(std::initializer_list<int> shape, MType fill_with) {
        assert(shape.size() == 4);
    ...
    template <typename MType>
    Matrix<MType>::Matrix(matrix_data_p data, matrix_dim shape) {
        assert(shape.size() == 4);
        long data_len {1};
        for(int dim : shape) {
            assert(dim >= 1);
    ...
    template <typename MType>
    Matrix<MType>& Matrix<MType>::operator*(int scale_number) {
        if(uninitialized) {
            throw std::runtime_error("Matrix is uninitialized, please initialized first");
        }
        scale(scale_number);
    ...
    Matrix<MType>& Matrix<MType>::add(const Matrix<MType>& addend) {
        ...
        if(this->data->size() == (addend.data)->size()) {
            ...
            int max_piece = data->size() / addend.data->size();
    ...
    void Matrix<MType>::add_boardcast_core(Matrix<MType>& summand, const Matrix<MType>& addend, ul_pos channel_ul, ul_pos a_channel_ul, int piece) {
        ...
        int max_piece = summand.data->size() / addend.data->size();
        ...
        int piece_w, piece_x, piece_y;
        piece_w = summand.shape[3] / addend_dim[3];
        piece_x = piece % piece_w; //x is w
        piece_y = piece / piece_w; // y is h
        for(int addend_h {0}; addend_h < addend_dim[2]; ++addend_h) {
            for(int addend_w {0}; addend_w < addend_dim[3]; ++addend_w) {
    ...
    template <typename MType>
    MType Matrix<MType>::get(int n, int c, int h, int w) {
    ...
    template <typename MType>
    MType Matrix<MType>::get(int index) {
        assert(index >= 0 && index < data->size());
        return data->at(index);
    }

    template <typename MType>
    void Matrix<MType>::set(int n, int c, int h, int w, MType value) {
        assert(n >= 0 && n < shape[0]);
        assert(c >= 0 && c < shape[1]);
        assert(h >= 0 && h < shape[2]);
        assert(w >= 0 && w < shape[3]);
    ...
    template <typename MType>
    void Matrix<MType>::set(int index, MType value) {
        assert(index < data->size());
        data->at(index) = value;
    }

    template <typename MType>
    typename Matrix<MType>::ul_pos Matrix<MType>::get_batch(matrix_dim dim, int batch_id) const {
        if(uninitialized) {
            throw std::runtime_error("Matrix is uninitialized, please initialized first");
        }
        assert(batch_id < dim[0]);
        long batch_len {dim[1] * dim[2] * dim[3]};
    ...
    template <typename MType>
    typename Matrix<MType>::ul_pos Matrix<MType>::get_batch(int batch_id) const {
        if(uninitialized) {
            throw std::runtime_error("Matrix is uninitialized, please initialized first");
        }
    ...
    template <typename MType>
    typename Matrix<MType>::ul_pos Matrix<MType>::get_channel(matrix_dim dim, int channel_id, ul_pos batch_pos) const {
        if(uninitialized) {
            throw std::runtime_error("Matrix is uninitialized, please initialized first");
        }
        assert(channel_id < dim[1]);
        long channel_len {dim[2] * dim[3]};
    ...
    template <typename MType>
    typename Matrix<MType>::ul_pos Matrix<MType>::get_channel(int channel_id, ul_pos batch_pos) const {
        if(uninitialized) {
            throw std::runtime_error("Matrix is uninitialized, please initialized first");
        }
    ...
    template <typename MType>
    Matrix<MType>& Matrix<MType>::scale(int scale_number) {
        if(uninitialized) {
            throw std::runtime_error("Matrix is uninitialized, please initialized first");
        }
        for(int i {0}; i < data->size(); ++i) {
    ...
    Matrix<MType>& Matrix<MType>::mul_v(const Matrix<MType> &mutiplier) {
        ...
        if(data->size() == mutiplier.data->size()) {
            ...
            std::vector<std::thread> thread_c;
            int max_piece = data->size() / mutiplier.data->size();
            for(int n {0}; n < shape[0]; ++n) {
                ul_pos this_batch_ul {get_batch(n)};
    ...
    void Matrix<MType>::mul_v_boardcast_core(Matrix<MType>& multiplied, const Matrix<MType>& mutiplier, ul_pos channel_ul, ul_pos m_channel_ul, int piece) {
        ...
        int max_piece = multiplied.data->size() / mutiplier.data->size();
        assert(piece < max_piece);
        ...
        piece_x = piece % piece_w; //x is w
        piece_y = piece / piece_w; // y is h
        for(int addend_h {0}; addend_h < mutiplier_dim[2]; ++addend_h) {
            for(int addend_w {0}; addend_w < mutiplier_dim[3]; ++addend_w) {
    ...
    template <typename MType>
    void Matrix<MType>::resize(int n, int c, int h, int w, MType fill_with) {
        assert(n >= 1);
        assert(c >= 1);
        assert(h >= 1);
        assert(w >= 1);
    ...
    template <typename MType>
    Matrix<MType> Matrix<MType>::sum_by_dim(int sum_dim) {
        if(sum_dim == -1) {
    ...
    template <typename MType>
    void Matrix<MType>::sum_by_dim_core(Matrix<MType>& m, Matrix<MType>& result, int sum_dim, int batch_id) {
        if(sum_dim == 1) {
            // channel sum
            for(int h {0}; h < m.shape[2]; ++h) {
                for(int w {0}; w < m.shape[3]; ++w) {
                    MType sum_value {0};
    ...
    template <typename MType>
    void Matrix<MType>::regard_as(matrix_dim shape) {
        shape_backup = this->shape;
        size_t new_len = shape[0] * shape[1] * shape[2] * shape[3];
    ...
    template <typename MType>
    void Matrix<MType>::regard_as(std::initializer_list<int> shape) {
        assert(shape.size() == 4);
        regard_as(matrix_dim {shape});
    }

    template <typename MType>
    void Matrix<MType>::restore_shape() {
        if(shape_backup[0] == 0) {
            throw std::runtime_error("`restore_shape` can not find any shape backup in this matrix");
        }
        shape = shape_backup;
        shape_backup = matrix_dim {0,0,0,0};
    }

    template <typename MType>
    bool Matrix<MType>::is_regarded_as() {
        if(shape_backup[0] != 0) {
            return true;
        }
        return false;
    ...
    void Matrix<MType>::T_core(ul_pos channel_ul) {
        ...
        int m_len {shape[2] * shape[3]};
        for(int i {0}; i < m_len; ++i) {
            int next_index = T_core_get_next(i, shape[2], m_len);
            while(i < next_index) {
                next_index = T_core_get_next(next_index, shape[2], m_len);
            }
            if(i == next_index) {               
                int next = T_core_get_next(i, shape[2], m_len);
    ...
    template <typename MType>
    int Matrix<MType>::T_core_get_next(int now_index, int m_h, int m_len) {
    ...
    template <typename MType>
    void Matrix<MType>::concat(Matrix<MType> &m, unsigned int dim) {
        assert(dim > 0 && dim <= 3);
    ...
    template <typename MType>
    void Matrix<MType>::concat_core(Matrix<MType>& m, Matrix<MType>& result, unsigned dim, int batch_id) {
        if(dim == 1) {
            ...
            for(int inner_c {0}; inner_c < shape[1]; ++inner_c) {
            ...
            for(int inner_c {0}; inner_c < m.shape[1]; ++inner_c) {
                for(int h {0}; h < shape[2]; ++h) {
                    for(int w {0}; w < shape[3]; ++w) {
            ...
            matrix_dim result_dim {result.shape};
            for(int c {0}; c < shape[1]; ++c) {
                for(int inner_h {0}; inner_h < shape[2]; ++inner_h) {
                    for(int w {0}; w < shape[3]; ++w) {
                        result.set(batch_id, c, inner_h, w, this->get(batch_id, c, inner_h, w));
                    }
                }
                for(int inner_h {0}; inner_h < m.shape[2]; ++inner_h) {
                    for(int w {0}; w < shape[3]; ++w) {
            ...
            for(int c {0}; c < shape[1]; ++c) {
                for(int h {0}; h < shape[2]; ++h) {
                    for(int inner_w {0}; inner_w < shape[3]; ++inner_w) {
                        result.set(batch_id, c, h, inner_w, this->get(batch_id, c, h, inner_w));
                    }
                    for(int inner_w {0}; inner_w < m.shape[3]; ++inner_w) {
    ...
    template <typename MType>
    Matrix<MType> Matrix<MType>::slice(int slice_dim, int start, int end) {
    ...
    template <typename MType>
    Matrix<MType> Matrix<MType>::slice(int slice_dim, std::initializer_list<int> range) {
        assert(range.size() == 2);
        slice_parma sr_dim {0,0,0,0,0,0,0,0};
    ...
    template <typename MType>
    Matrix<MType> Matrix<MType>::slice(std::initializer_list<int> slice_index) {
    ...
    Matrix<MType> Matrix<MType>::slice(slice_parma slice_index) {
        ...
        int sw_index {0};
        for(int n {slice_index[0]}; n < slice_index[1]; ++n) {
            for(int c {slice_index[2]}; c < slice_index[3]; ++c) {
                for(int h {slice_index[4]}; h < slice_index[5]; ++h) {
                    for(int w {slice_index[6]}; w < slice_index[7]; ++w) {
